...
//        static::doNotSnakeAttributes($this->model);
    ...
    public function buildDataWithCompany( $value, $toArray = true)
    {
        //print_r($value);
       // die;
        if (empty($this->data)) {
            ...
            if($this->getTable() ==  with(new Company)->getTable()){
                 $companyColumn = '.id';
            }
            
            if($this->getTable() == with(new ShopActivity)->getTable()){
                $companyColumn = '.id';
                $value = Company::find($value)->activity_id;
           }

            $collection = $this->where($this->getTable().$companyColumn, $value)->get();
