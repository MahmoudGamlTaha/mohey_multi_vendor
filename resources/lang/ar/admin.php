<?php

return [
    'online'                => 'متصل',
    'Imglocation'           => 'مكان الصورة',
    'login'                 => 'تسجيل الدخول',
    'logout'                => 'تسجيل الخروج',
    'setting'               => 'الضبط',
    'name'                  => 'الاسم',
    'email'                 => 'البريد الالكتروني',
    'username'              => 'اسم المستخدم',
    'password'              => 'الرقم السري',
    'password_confirmation' => 'تأكيد الرقم السري',
    'remember_me'           => 'تذكرني',
    'user_setting'          => 'ضبط المستخدم',
    'avatar'                => 'الصورة',
    'list'                  => 'القائمة',
    'new'                   => 'جديد',
    'create'                => 'انشاء',
    'delete'                => 'مسح',
    'remove'                => 'حذف',
    'edit'                  => 'تعديل',
    'type'                  => 'نوع النشاط',
    'view'                  => 'عرض',
    'continue_editing'      => 'مواصلة التحرير',
    'continue_creating'     => 'تكملة انشاء',
    'detail'                => 'التفاصيل',
    'browse'                => 'تصفح',
    'reset'                 => 'تفريغ',
    'export'                => 'تصدير',
    'batch_delete'          => 'مسح بالجملة',
    'save'                  => 'حفظ',
    'refresh'               => 'تحديث',
    'order'                 => 'ترتيب',
    'expand'                => 'تكبير',
    'collapse'              => 'تصغير',
    'filter'                => 'تصنيف',
    'search'                => 'بحث',
    'close'                 => 'اغلاق',
    'show'                  => 'عرض',
    'entries'               => 'المدخلات',
    'captcha'               => 'كود التحقق',
    'action'                => 'الحدث',
    'title'                 => 'العنوان',
    'description'           => 'الوصف',
    'back'                  => 'عودة',
    'back_to_list'          => 'العودة الى القائمة',
    'submit'                => 'ارسال',
    'menu'                  => 'القائمة',
    'input'                 => 'المدخل',
    'succeeded'             => 'تمت بنجاح',
    'failed'                => 'فشل',
    'delete_confirm'        => 'هل انت متاكد من مسح هذا العنصر ',
    'delete_succeeded'      => 'تم الحذف بنجاح ! ',
    'delete_failed'         => 'فشل الحذف !',
    'update_succeeded'      => 'تم التعديل بنجاح !',
    'save_succeeded'        => 'تم الحفظ !',
    'refresh_succeeded'     => 'تم التحديث !',
    'login_successful'      => 'تم تسجيل الدخول بنجاح',
    'choose'                => 'اختر',
    'choose_file'           => 'اختر الملف',
    'choose_image'          => 'اختر الصورة',
    'more'                  => 'المزيد',
	'image'                 => 'صورة',
    'deny'                  => 'ليس لديك الاذن',
    'activity'             =>  'النشاط',
    'administrator'         => 'مسئول',
    'roles'                 => 'الادوار',
    'permissions'           => 'الصلاحيات',
    'company'               => 'الشركة',
    'seller'              =>  'بائع',
    'companies'             => 'الشركات',
    'slug'                  => 'المعرف',
    'created_at'            => 'تاريخ الانشاء',
    'updated_at'            => 'تاريخ التعديل',
    'alert'                 => 'تحذير',
    'parent_id'             => 'الاصل',
    'icon'                  => 'الرمز',
    'units'                 => 'الوحدات',
    'uri'                   => 'URI',
    'operation_log'         => 'سجل التشغيل',
    'parent_select_error'   => 'خطأ في تحديد الاصل',
    'iban'                => 'IBAN', //sprint 1
    'pagination'            => [
        'range' => 'عرض :first الى :last من :total المدخلات',
    ],
    'role'                  => 'القاعدة',
    'permission'            => 'الصلاحية',
    'route'                 => 'المسار',
    'confirm'               => 'تأكيد',
    'cancel'                => 'إلغاء',
    'http'                  => [
        'method' => 'HTTP method',
        'path'   => 'HTTP path',
    ],
    'all_methods_if_empty'  => 'كل الوسائل فارغة',
    'all'                   => 'الكل',
    'current_page'          => 'الصفحة الحالية',
    'selected_rows'         => 'الصفوف المختارة',
    'upload'                => 'رفع',
    'new_folder'            => 'مجلد جديد',
    'time'                  => 'الوقت',
    'size'                  => 'الحجم',
    'listbox'               => [
        'text_total'         => 'عرض الكل {0}',
        'text_empty'         => 'قائمة فارغة',
        'filtered'           => '{0} / {1}',
        'filter_clear'       => 'عرض الكل',
        'filter_placeholder' => 'تنقية',
        'color'               =>       'اللون',
        'carton'              =>       'كرتونة',
        'pocket'              =>       'علبة',
        'tanks'               =>      'شكارة',
        'area'                => 'المساحة'
    ],
    'menu_titles'           => [
    'admin'                 =>   'الإدارة',
     'menu'                 => 'القائمة',
     'api_manager'          =>   'مدير التطبيقات',
     'shop_api'             =>   'مدير المتجر',
     'currencies'           =>   'العملات',
     'operation_log'        => 'سجل التشغيل',
     'pages'                => 'الصفحات',
     'localisation'         =>   'الموقع',
     'layout'               =>  'التصميم',
     'config_language'      =>   'لغة التكوين',
     'config_global'        => 'التكوين العالمي',
     'banners'              => 'اللافتات',
     'config_info'          => 'معلومات التكوين',
	 'categories'       	=>   'التصنيف',
	 'product_manager'      =>   'ادارة المنتجات',
	 'index'                =>   'اللوحة الرئسية',
	 'order_manager'        =>   'إدارة الطلبات',
	 'customer_manager'     =>   'إدارة العملا',
	 'all_products'         =>   'كل المنتجات',
	 'special_price'        =>   'عروض خاصة',
	 'brand'                =>   'ماركات',
     'import_multi_product' =>   'ادخال مجموعة منتجات',
     'images_manager'       =>   'مدير الصور',
     'images'               =>   'الصور',
	 'vendor'               =>   'الموردين',
	 'attributes_group'     =>   'ملحقات المنتج',
	 'group_name'           =>   'الاسم',
	 'orders'               =>   'الطلبات',
	 'order_status'         =>   'حالات الطلب',
	 'payment_status'       =>   'انواع الدفع',
	 'shipping_status'      =>   'حالات الشحن',
     'customer'             =>   'العملاء',
     'customers'            =>   'العملاء',
     'product'              =>   'المنتج',
	 'payment'              =>   'الدفع',
	 'shipping'             =>   'الشحن',
     'total'                =>    'الخصم',
     'modules'              =>    'الوحدات',
     'cms'                  =>    'cms',
	 'other'                =>    'اخرى',
	 'extensions'           =>    'الاضافات',
     'settings'             =>    'الاعدادات',
     'URL'                  =>    'URL',
	 'report_&_analytics'   =>   'التقارير',
     'customers_category'  =>    'تصنيفات العملاء',
     'subscribe_manager'   =>    'مدير الاشتراك',
     'users'               =>     'الاعضاء',
     'backup_&_restore'    =>      'استرجاع البيانات',
	 'permission'          =>      'الصلاحيات',
     'roles'               =>       'الأدوار',
     'templates_manager'   =>       'مدير القوالب',
     'companies'           =>       'الشركات',
     'design_layout'       =>       'تصميم القالب',
     'activities'          =>       'الأنشطة',
     'pricelist'           =>       'قائمة الاسعار',
     'others'              =>    'اخرى',
     'paymentterm'         =>    'شروط الدفع',
     'units'                 => 'الوحدات',


	],
];
