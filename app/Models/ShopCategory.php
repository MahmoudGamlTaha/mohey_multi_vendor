 ...
 */
    public function getCategoriesAll($homePage = false, $all = true, $sortBy = null, $sortOrder = 'asc')
    {
        if ($all) {
            if(!$homePage){
            $listFullCategory = $this->sort($sortBy, $sortOrder)->get()->groupBy('parent');
                ...
                $listFullCategory = $this->sort($sortBy, $sortOrder)->where('home_page', true)->get()->groupBy('parent');
            }
        } else {
