...
Route::group([
    ...
    $router->resource('payment_term', PaymentTermController::class)->names('paymentTermControl');
    $router->resource('customer_payment_term', CustomerPaymentTermController::class)->names('customerPaymentControl');
//Config
...
//Get info
   ...
   $router->group(['prefix' => 'item_units'], function ($router) {
       $router->post('/{id}/addNewUofm','UnitOfMeasureController@addUnit');
   });
